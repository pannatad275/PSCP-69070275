...
```text
โจทย์กำหนดมาว่าจะมีน้ำหมึกหยดลงมาแล้วแพร่ออกเป็นวงกลมทีละSต่อวิ ให้หาว่ามีครอบครัวไหนที่จะโดนน้ำท่วมตอนxวินาที
```
...
```text
ตั้งตัวแปรมาเก็บข้อมูลของระยะพื้นที่ที่หมึกจะขยายออก(S) และตัวแปรเก็บข้อมูลว่ามีคนขอความช่วยเหลือกี่คน(N)
แล้วตั้งfor loop กำหนดให้สุดแค่(N) แล้วสร้างinputเก็บพิกัดบ้าน(X,Y)ตามจำนวนคน(N) แล้วคำนวณหาพื้นที่ของวงกลมเมื่อถึงบ้านที่ตั้งอยุ่ printออกมาใช้คำสั่งm.ceilเพื่อปัดตัวเลขขึ้น
